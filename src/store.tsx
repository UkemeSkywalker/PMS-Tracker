...
type AppState = {
  ...
  submit: (id: string, price: number, availability: Availability, queueStatus: QueueStatus, verificationPhotoUri?: string) => void;
};
...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<AppState>(() => ({
    ...
    submit: (id, price, availability, queueStatus, verificationPhotoUri) => {
      setStations(previous => previous.map(item => item.id === id ? { ...item, pmsPrice: price, availability, queueStatus, verificationPhotoUri, lastUpdated: new Date().toISOString(), reportCount: item.reportCount + 1, verified: true, trend: [...item.trend.slice(1), price] } : item));
